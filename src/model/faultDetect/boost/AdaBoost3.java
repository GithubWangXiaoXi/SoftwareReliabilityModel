...
/**
 * AdaBoost3在给测试样本分类时，不依赖于给定的规则强硬分类，而是通过训练数据集在规则上的概率，通过阈值灵活分类
 * 叶子节点只需保留正类即可。
 */
public class AdaBoost3 implements BasicBoost {
...
//            G_x = getG_x();
    ...
    public int[] getoutdata(double thresholdT) {
        ...
        for (int i = 0; i < x_test.length; i++) {
            int result = predict(root_m, a, x_test[i],i,thresholdT);
            results[i] = result;
            System.out.println("真实标签：" + y_test[i] + " 预测标签：" + results[i]);
        }
    ...
    public double createDecisionTree(BTNode root, LinkedList<Integer> list, double [][]eTemp, int m, Set<Integer> featureSelected, boolean leftT,boolean rightT){
        ...
        if(!flag){
            ...
            //添加预测为正类的叶子节点(原来的leftChild，rightChild无需置为空，因为需要计算em）
            if(root.floor == dimensions){

                BTNode leafPosNode = new BTNode();

                root.leafPosNode = leafPosNode;

                //右节点为正类
                if(root.borderLeft < 0){
                    root.leafPosNode.probability = (double) x_list2.size() / list.size();
                }
                //左节点为正类
                else{
                    root.leafPosNode.probability = (double) x_list1.size() / list.size();
                }
        ...
        //该root节点本来就是叶子节点（没有训练数据落在它上面，但是还需要为它添加概率节点）
        else{
            root.isLeaf = true;
            if(leftT){
                root.isLeftChild = true;
            ...
            //如果该叶结点为正类，则让它的parent再生出个leafPosNode小宝宝
            BTNode leafPosNode = new BTNode();
            root.leafPosNode = leafPosNode;
            ...
            if(root.isRightChild){
                //如果root为正类，则概率为1.0
                if(root.parent.borderLeft < 0){
                    root.leafPosNode.probability = 1.0;
                }
                //如果root为负类，则概率为0.0
                else{
                    root.leafPosNode.probability = 0.0;
                }
            }
            //该叶结点为左节点
            else{
    ...
    //通过设置阈值threshold，与概率节点比较。如果大于阈值，则标为正类，如果小于阈值，则标为负类
    private int predict(LinkedList<BTNode> root_m, double[] a, double[] sample, int index,double thresholdT) {
        ...
        for (int i = 0; i < root_m.size(); i++) {
            ...
            /**
             * 通过p的parent获取概率节点leafPosNode
             * 1)如果概率 > threshold，则该决策树预测结果为+1
             * 2)如果概率 < threshold，则该决策树预测结果为-1
             * 3)累加该决策树预测结果 * a_m[i]
             */
            if(p.parent.leafPosNode != null && p.parent.leafPosNode.probability > thresholdT){
                results += a[i] * 1;
            }else{
                results += a[i] * (-1);
            }
...
//        }
        ...
        while (p.next != null){
            ...
            if(p.parent.leftChild != null && p.isLeftChild){
                ...
                if(p.parent.leafPosNode != null && p.parent.isPrint == false){
                    System.out.print("正类概率: " + p.parent.leafPosNode.probability + "  ");
                    p.parent.isPrint = true;
                }
            }
            ...
            //如果叶子节点不在最后一层，且叶子节点的概率节点不空时，打印输出
            if(p.floor != dimensions + 1 && p.leafPosNode != null) {
                System.out.print("正类概率: " + p.leafPosNode.probability + "  ");
            }
        }
        System.out.println();
    }
    ...
    public int[] getoutdata() {
        ...
        int results[] = new int[x_test.length];
        for (int i = 0; i < x_test.length; i++) {
            int result = predict1(root_m, a, x_test[i],i);
            results[i] = result;
        }
    ...
    //预测测试样本
    private int predict1(LinkedList<BTNode> root_m, double[] a, double[] sample,int index) {
