...
/**
 * AdaBoost分析一维特征，得到学习模型（单层树）
 * note: AdaBoost没必要实现BasicModel，但是这么写方便通过BasicModel接口获取接口的实现类（存在多个），便于拓展
 */

public class AdaBoost implements BasicBoost {
    ...
    private double []e;  //每个弱分类器对应的最小误分类率
        ...
        //非第一次迭代，则插入原有的分段函数
            ...
            //在表内插入
                ...
                if(xMap.get(borderIndex) != null){
                    ...
                    for (int i = 0; i <= x_borderList.indexOf(borderIndex); i++) {
                        y_borderList.set(i, borderleft + y_borderList.get(i));
                    }

                    //borderRight + y插入位置后所有元素
                    for (int i = x_borderList.indexOf(borderIndex) + 1; i <= x_borderList.size(); i++) {
                        y_borderList.set(i, borderRight + y_borderList.get(i));
                    }
                }
                else{
    ...
    public int getMissPointCount() {
        ...
        for (int i = 0; i < x_train.length; i++) {
            ...
            if(x_train[i][0] < x_borderList.getFirst()) {
                if(isMissPoint(y_train[i],y_borderList.getFirst(),i)) {
                    missPointCount++;
                }
            }
            //样本x值在G_x分段函数的末尾
            else if(x_train[i][0] > x_borderList.getLast()) {
                if(isMissPoint(y_train[i],y_borderList.getLast(),i)) {
                    missPointCount++;
                }
            }
            //样本x值在G_x分段函数的中间
                ...
                for(int j = 0; j < x_borderList.size(); j++) {
                    if (x_borderList.get(j) > x_train[i][0]) {
                        double val = y_borderList.get(j);
                        if(isMissPoint(y_train[i],val,i)){
                            missPointCount++;
                        }
                        break;
                    }
    ...
    public boolean isMissPoint(int y_real, double y_predict,int index) {
