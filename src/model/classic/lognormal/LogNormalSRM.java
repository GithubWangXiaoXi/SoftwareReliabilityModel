package model.classic.lognormal;

import modelinterface.ClassicModel;
import phsrm.common.*;

public class LogNormalSRM implements ClassicModel {
	...
	public double[] getPDF(double t[]) {
		...
		PDF[i]= Numlib.d_normal((Math.log(t[i])-p)/q)/(t[i]*q);}
		 return PDF;
	}
	...
	public double[] getCDF(double t[]) {
		...
		for (int i = 0; i < t.length; i++) {
			CDF[i]=Numlib.p_normal((Math.log(t[i])-q)/p);
	}
		return CDF;
	}
	...
	private double[] Fault(double a[]) {
		...
		for (int i = 1 ; i < a.length; i++) {
			...
			fault[i] = (Math.log(a[i])-q)/p;
		//	fault[i] = getPDF(a)[i]/(1-getCDF(a)[i]);
		}
		return fault.clone();
	}
