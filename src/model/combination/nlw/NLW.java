...
public class NLW implements CombinationModel
{
	...
	public void inputdata(double data[],int step,String[] modelselect,double[][] parameter)
	{
		...
		for(int i=0;i<predictdata_x.length;i++)
		{
			...
			{
				if(predictdata[j][i] <= 0) predictdata_x[i][j]=0;
				else predictdata_x[i][j] = Math.log(predictdata[j][i]);
			}
		}
		...
		{
			data_x[i] = new double[fitnessdata[count+i].length];
			for(int j=0;j<data_x[i].length;j++)
			{
				if(fitnessdata[count+i][j] <=0 ) data_x[i][j]=0;
				else data_x[i][j] = Math.log(fitnessdata[count+i][j]);
			}
			if(data[count+i] <=0 ) result[i]=0;
			else result[i] = Math.log(data[count+i]);
		}
		...
		for(int i=0;i<fitness_x.length;i++)
		{
			fitnessdata_combination[count+i] =
					Double.parseDouble(df.format(Math.pow(Math.E, fitness_x[i])));
		}
		outdata = basicModel_FunctionMining.getoutdata2(step, predictdata_x);
		for(int i=0;i<outdata.length;i++)
		{
			outdata[i] = Math.pow(Math.E, outdata[i]);
		}
