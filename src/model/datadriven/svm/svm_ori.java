...
public class svm_ori {
					...
					double Cp, double Cn)
	{
		...
		if (Cp==Cn)
			svm_ori.info("nu = "+sum_alpha/(Cp*prob.l)+"\n");
					...
					double[] alpha, Solver.SolutionInfo si)
	{
		...
		svm_ori.info("nu = "+sum_alpha/(param.C*l)+"\n");
	}
		...
		double Cp, double Cn)
	{
		...
		svm_ori.info("obj = "+si.obj+", rho = "+si.rho+"\n");
		svm_parameter.obj = si.obj;
		svm_parameter.rh0 = si.rho;
		...
		svm_ori.info("nSV = "+nSV+", nBSV = "+nBSV+"\n");
		svm_parameter.nSV = nSV;
		svm_parameter.nBSV = nBSV;
				  ...
				  double[] probAB)
	{
		...
		for (iter=0;iter<max_iter;iter++)
		{
			...
			{
				svm_ori.info("Line search fails in two-class probability estimates\n");
				break;
			}
		}
		
		if (iter>=max_iter)
			svm_ori.info("Reaching maximal iterations in two-class probability estimates\n");
	...
	private static void multiclass_probability(int k, double[][] r, double[] p)
	{
		...
		if (iter>=max_iter)
			svm_ori.info("Exceeds max_iter in multiclass_prob\n");
	}
	...
	private static double svm_svr_probability(svm_problem prob, svm_parameter param)
	{
		...
		for(i=0;i<prob.l;i++)
			if (Math.abs(ymv[i]) > 5*std) 
				...
				mae+=Math.abs(ymv[i]);
		mae /= (prob.l-count);
		svm_ori.info("Prob. model for test data: target value = predicted value + z,\nz: Laplace distribution e^(-|z|/sigma)/(2sigma),sigma="+mae+"\n");
		return mae;
	}
	...
	public static svm_model svm_train(svm_problem prob, svm_parameter param)
	{
		...
		else
		{
			...
			if(nr_class == 1) 
				svm_ori.info("WARNING: training data in only one class. See README for details.\n");
