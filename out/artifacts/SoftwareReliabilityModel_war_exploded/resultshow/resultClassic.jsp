...
<jsp:useBean id="calculate_classic" class="calculate.CalculateClassic" 
	...
	<body onload="onload();">
 	...
 	<%	}	
 		else
 		{
			...
			calculate_classic.inputdata(data_train, model, step, parameter);
 			double[] outdata = calculate_classic.getoutdata();
			double[] fitness = calculate_classic.getfitness();
 			String process = calculate_classic.getprocess();
 			String str_parameterinfo = calculate_classic.getparameterinfo();
 			String date = calculate_classic.getdate();
 			double[] u = calculate_classic.get_ugraph();
 			double[] y = calculate_classic.get_ygraph();
    	...
    	%>
 			<div class="show_tab_menu">
    			<div style="width:100%;">
					<ul> 
						<li id="resultC_tab1" onClick="setTab('resultC',1,9)" class="hover">预测数据</li>
						<li id="resultC_tab2" onClick="setTab('resultC',2,9)">拟合数据</li>
						<li id="resultC_tab3" onClick="setTab('resultC',3,9)">K-S检验</li>
						<li id="resultC_tab4" onClick="setTab('resultC',4,9)">参数估计</li>
						<li id="resultC_tab5" onClick="setTab('resultC',5,9)">U-结构图</li>
						<li id="resultC_tab6" onClick="setTab('resultC',6,9)">Y-结构图</li>
						<li id="resultC_tab7" onClick="setTab('resultC',7,9)">性能参数</li>
						<li id="resultC_tab8" onClick="setTab('resultC',8,9)">残差分析</li>
						<li id="resultC_tab9" onClick="setTab('resultC',9,9)">拟合-预测图</li>
					</ul> 
				</div>
				<div class="subtitle">
					完成时间:<br>
    		...
    		<div class="show_tab_content">
    			...
    			<div id="resultC_con2" style="display:none">
  	 					...
  	 					<% 	for(int j=0;j<data_train.length;j++)
							{%>
  	 						<tr <% if(j%2==0) {%>class="altrow"<%}%>>
  	 							...
  	 							<td><%= data_train[j]%></td>
  	 							<td>
  	 							<%	if(fitness[j]==0.00001) out.print("-");
				...
				<div id="resultC_con3" style="display:none;text-align:center;">
				...
				<div id="resultC_con4" style="display:none">
					<div class="subtitle">参数估计</div>
					<% 
						process = process.replaceAll("\n", "<br>");
					%>
					<div class="scrollbox" style="font-size:18px"><%= process%></div>
				</div>
				<div id="resultC_con5" style="display:none;text-align:center;">
				<%	filename = uychart.generateLineChart(u,"U",session,new PrintWriter(out));
		     		graphURL = request.getContextPath() + "/DisplayChart?filename=" + filename;
		     	%>
						<img src="<%= graphURL %>">
				</div>
				<div id="resultC_con6" style="display:none;text-align:center;">
				<%	filename = uychart.generateLineChart(y,"Y",session,new PrintWriter(out));
		     			graphURL = request.getContextPath() + "/DisplayChart?filename=" + filename;
		     	%>
					<img src="<%= graphURL %>">
				</div>
				<div id="resultC_con7" style="display:none">
				...
				<div id="resultC_con8" style="display:none;text-align:center;">
					<%	if(data_test.length==0)
						{%>
